...
`define AESL_DEPTH_vCol 1
`define AESL_DEPTH_vRow 1
...
`define AUTOTB_TVIN_vCol_out_wrapc  "../tv/rtldatafile/rtl.kernel.autotvin_vCol.dat"
`define AUTOTB_TVIN_vRow_out_wrapc  "../tv/rtldatafile/rtl.kernel.autotvin_vRow.dat"
...
parameter LENGTH_vCol = 1;
parameter LENGTH_vRow = 1;
...
// The signal of port vCol
reg [31: 0] AESL_REG_vCol = 0;
assign vCol = AESL_REG_vCol;
initial begin : read_file_process_vCol
    integer fp;
    integer err;
    integer ret;
    integer proc_rand;
    ...
    transaction_idx = 0;
    wait(AESL_reset === 1);
    fp = $fopen(`AUTOTB_TVIN_vCol,"r");
    if(fp == 0) begin       // Failed to open file
        $display("Failed to open file \"%s\"!", `AUTOTB_TVIN_vCol);
    ...
    while (token != "[[[/runtime]]]") begin
        ...
        if(token != "[[/transaction]]") begin
            ret = $sscanf(token, "0x%x", AESL_REG_vCol);
              if (ret != 1) begin
...
// The signal of port vRow
reg [31: 0] AESL_REG_vRow = 0;
assign vRow = AESL_REG_vRow;
initial begin : read_file_process_vRow
    integer fp;
    integer err;
    integer ret;
    integer proc_rand;
    ...
    transaction_idx = 0;
    wait(AESL_reset === 1);
    fp = $fopen(`AUTOTB_TVIN_vRow,"r");
    if(fp == 0) begin       // Failed to open file
        $display("Failed to open file \"%s\"!", `AUTOTB_TVIN_vRow);
        $display("ERROR: Simulation using HLS TB failed.");
        $finish;
    end
    read_token(fp, token);
    ...
    while (token != "[[[/runtime]]]") begin
        ...
        if(token != "[[/transaction]]") begin
            ret = $sscanf(token, "0x%x", AESL_REG_vRow);
              if (ret != 1) begin
                  $display("Failed to parse token!");
                $display("ERROR: Simulation using HLS TB failed.");
                  $finish;
...
reg end_vCol;
reg [31:0] size_vCol;
reg [31:0] size_vCol_backup;
reg end_vRow;
reg [31:0] size_vRow;
reg [31:0] size_vRow_backup;
