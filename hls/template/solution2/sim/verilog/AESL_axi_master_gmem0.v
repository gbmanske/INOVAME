...
//------------------------Parameter----------------------
`define TV_IN_gmem0 "../tv/cdatafile/c.kernel.autotvin_gmem0.dat"
`define TV_OUT_gmem0 "../tv/rtldatafile/rtl.kernel.autotvout_gmem0.dat"
 parameter gmem0_ADDR_BITWIDTH = 32'd 64;
 parameter gmem0_AWUSER_BITWIDTH = 32'd 1;
 parameter gmem0_DATA_BITWIDTH = 32'd 1024;
...
parameter gmem0_C_DATA_BITWIDTH = 32'd 1024;
parameter gmem0_transaction_depth = 32'd 8192;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem0_mem_depth) begin
                $display ("/home/gme/guilherme.manske/hls/template/solution2/sim/verilog/AESL_axi_master_gmem0.v: Write request address %d exceed AXI master gmem0 array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem0_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem0_mem_depth) begin
                $display ("/home/gme/guilherme.manske/hls/template/solution2/sim/verilog/AESL_axi_master_gmem0.v: Read request address %d exceed AXI master gmem0 array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem0_mem_depth); 
                $finish;
            end
...
//------------------------Task and function--------------
 ...
 function is_blank_char(input [7:0] in_char);
    ...
    function [2071:0] read_token(input integer fp);
        integer ret;
        begin
            read_token = "";
                    ret = 0;
 ...
 function [2071:0] rm_0x(input [2071:0] token);
...
task write_binary;
    input integer fp;
    input reg[1024-1:0] in;
    input integer in_bw;
    reg [63:0] tmp_long;
    reg[1024-1:0] local_in;
    integer char_num;
    integer long_num;
    integer i;
    integer j;
...
initial begin : write_file_proc 
  ...
  while(1) begin 
      ...
      bin_data = gmem0_transaction_depth;
      write_binary(fp,bin_data,64);
      for(i = 0; i < gmem0_mem_depth; i = i + 1) begin 
          case(mem_page)
              0 : tmp_data = gmem0_mem_0[i];
              1 : tmp_data = gmem0_mem_1[i];
              2 : tmp_data = gmem0_mem_2[i];
              3 : tmp_data = gmem0_mem_3[i];
              default: $display("The page_num of write file is not valid!");
          endcase
          
          for (j=0;j<1;j=j+1) begin
              case(j%1)
                  0: mem_tmp = tmp_data[1023:0];
              endcase
              bin_data = mem_tmp;
              write_binary(fp,bin_data,1024);
          end
      end
